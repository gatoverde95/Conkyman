...
# Centralize the retrieval of the .conkyrc file path
def get_conkyrc_path():
    return os.path.expanduser('~') + '/.conkyrc'

# Centralize the show_dialog function
...
themes_folder = os.path.join(script_dir, '/usr/share/conkyman/themes_en')

if not os.path.exists(themes_folder):
    raise FileNotFoundError(f'The "themes_en" folder was not found in {themes_folder}')
...
def replace_conkyrc(theme_name):
    ...
    try:
        ...
        show_dialog("Notice!", f'Theme "{theme_name}" applied successfully.', Gtk.MessageType.INFO)
        restart_conky()
    except Exception as e:
        show_dialog("Notice!", f'Error applying theme: {str(e)}', Gtk.MessageType.ERROR)
...
def is_conky_running():
    try:
        ...
        show_dialog("Notice!", f'Error checking Conky: {str(e)}', Gtk.MessageType.ERROR)
        return False
...
def kill_conky():
    try:
        ...
        show_dialog("Notice!", f'Error killing Conky: {str(e)}', Gtk.MessageType.ERROR)
...
def restart_conky():
    ...
    try:
        ...
        show_dialog("Notice!", 'Conky restarted successfully.', Gtk.MessageType.INFO)
    except Exception as e:
        show_dialog("Notice!", f'Error restarting Conky: {str(e)}', Gtk.MessageType.ERROR)

def start_conky():
    is_running = is_conky_running()
    if is_running:
        show_dialog("Notice!", 'Conky is already running.', Gtk.MessageType.INFO)
    else:
        try:
            subprocess.Popen(["conky", "-d"], start_new_session=True)
            show_dialog("Notice!", 'Conky started successfully.', Gtk.MessageType.INFO)
        except Exception as e:
            show_dialog("Notice!", f'Error starting Conky: {str(e)}', Gtk.MessageType.ERROR)
...
class ConkymanWindow(Gtk.Window):
    def __init__(self):
        ...
        themes_box.pack_start(Gtk.Label(label="Select a theme:"), False, False, 0)
        ...
        notebook.append_page(themes_box, Gtk.Label(label="Themes"))
        ...
        position_box.pack_start(Gtk.Label(label="General Position:"), False, False, 0)
    ...
    def check_conkyrc(self):
        ...
        if not os.path.exists(conkyrc_path):
            dialog = Gtk.MessageDialog(
                ...
                text='.conkyrc file not found. Do you want to generate it automatically?'
            )
    ...
    def generate_conkyrc(self):
        ...
        try:
            ...
            show_dialog("Notice!", '.conkyrc file generated successfully.', Gtk.MessageType.INFO)
            start_conky()
        except Exception as e:
            show_dialog("Notice!", f'Error generating .conkyrc file: {str(e)}', Gtk.MessageType.ERROR)
    ...
    def move_conky(self, alignment, gap_x, gap_y):
        conkyrc_path = get_conkyrc_path()
        if not os.path.exists(conkyrc_path):
            show_dialog("Notice!", f'.conkyrc file not found in {conkyrc_path}.', Gtk.MessageType.ERROR)
            return
        ...
        try:
            ...
            show_dialog("Notice!", 'Conky position updated successfully.', Gtk.MessageType.INFO)
            restart_conky()
        except Exception as e:
            show_dialog("Notice!", f'Error updating position: {str(e)}', Gtk.MessageType.ERROR)
    ...
    def show_about_dialog(self, widget):  # pylint: disable=unused-argument
        ...
        about_dialog.set_comments("Conky theme manager for CuerdOS GNU/Linux.")
